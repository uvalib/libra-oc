...
# install ruby
RUN cd /tmp && wget https://cache.ruby-lang.org/pub/ruby/2.5/ruby-2.5.1.tar.gz
RUN cd /tmp && tar xzvf ruby-2.5.1.tar.gz
RUN cd /tmp/ruby-2.5.1 && ./configure && make && make install
RUN rm -fr /tmp/ruby-2.5.1 && rm /tmp/ruby-2.5.1.tar.gz

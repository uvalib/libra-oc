...
class PublicViewController < ApplicationController
  ...
  def setup_meta_tags
    set_meta_tags(
      ...
      "DC.issued": (Date.parse(@work.published_date).try(:strftime, "%Y/%-m/%-d") if @work.published_date),
      citation_online_date: (Date.parse(@work.date_created).try(:strftime, "%Y/%-m/%-d") if @work.date_created),
      "DC.identifier": @work.identifier,
